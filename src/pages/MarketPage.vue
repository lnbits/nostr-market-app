...
export default defineComponent({
    ...
    filterProducts() {
      ...
      return products.filter(
        (p) =>
          ...
          (p.categories && p.categories.toString().toLowerCase().includes(searchText))
      );
    },
    filterStalls() {
      ...
      return this.stalls.filter(
        (s) =>
          ...
          (s.categories && s.categories.toString().toLowerCase().includes(searchText))
      );
    },
    stallName() {
  ...
  methods: {
    ...
    restoreFromStorage() {
      ...
      const uiConfig = this.$q.localStorage.getItem("nostrmarket.marketplaceConfig") || {
        ui: { darkMode: false },
      };
    ...
    handlePaymentRequest(json) {
      ...
      const paymentRequest = json.payment_options.find((o) => o.type == "ln").link;
    ...
    persistDMEvent(event) {
      const dms = this.$q.localStorage.getItem(`nostrmarket.dm.${event.pubkey}`) || {
        events: [],
        lastCreatedAt: 0,
      };
